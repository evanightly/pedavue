...
export default function ModuleStageEdit({ record }: ModuleStageEditProps) {
  ...
  const [moduleContentId, setModuleContentId] = useState<number | string | null>(() => {
    const direct = record?.module_content_id;

    if (typeof direct === 'number' || typeof direct === 'string') {
      return direct;
    }

    const related = record?.module_content;
  ...
  const [moduleQuizId, setModuleQuizId] = useState<number | string | null>(() => {
    const direct = record?.module_quiz_id;

    if (typeof direct === 'number' || typeof direct === 'string') {
      return direct;
    }

    const related = record?.module_quiz;
  ...
  return (
    <AppLayout>
      <Head title="Edit Module Stage" />
      <Form {...ModuleStageController.update.form(record.id)}
        ...
        {({ errors, processing }) => (
          <div className="space-y-6 rounded-xl border bg-card p-8 shadow-sm">
            <div className="space-y-2">
              <h1 className="text-2xl font-semibold tracking-tight">Edit Module Stage</h1>
            ...
            <div className="grid gap-6">
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  type="text"
                  required
                  defaultValue={normalizeFieldValue(record.module_able)}
                />
