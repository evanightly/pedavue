...
interface QuizResultEditProps {
  record: QuizResultRecord;
}

export default function QuizResultEdit({ record }: QuizResultEditProps) {
  ...
  const [quizId, setQuizId] = useState<number | string | null>(() => {
    const direct = record?.quiz_id;

    if (typeof direct === 'number' || typeof direct === 'string') {
      return direct;
    }

    const related = record?.quiz;
  ...
  return (
    <AppLayout>
      <Head title="Edit Quiz Result" />
      <Form {...QuizResultController.update.form(record.id)}
        ...
        {({ errors, processing }) => (
          <div className="space-y-6 rounded-xl border bg-card p-8 shadow-sm">
            <div className="space-y-2">
              <h1 className="text-2xl font-semibold tracking-tight">Edit Quiz Result</h1>
            ...
            <div className="grid gap-6">
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  type="number"
                  required
                  defaultValue={normalizeFieldValue(record.score)}
                />
              ...
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  type="number"
                  required
                  defaultValue={normalizeFieldValue(record.attempt)}
                />
