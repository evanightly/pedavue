...
export default function RoleIndex({ roles, filters = null, sort = null, filteredData: initialFilteredData = null }: RoleIndexProps) {
    ...
    const columnFilters =
        filters?.columnFilters && typeof filters.columnFilters === 'object' && !Array.isArray(filters.columnFilters)
            ...
            : {};
    const columns = useMemo<(ColumnDef<RoleRecord> & ColumnFilterMeta)[]>(
        () => [
            {
                id: 'id',
                accessorKey: 'id',
                        ...
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent align='end' className='w-40'>
                                <DropdownMenuItem asChild>
                                    <Link href={RoleController.show(row.original.id).url} className='flex items-center gap-2 text-sm'>
                                        View
                                    </Link>
                                </DropdownMenuItem>
                                <DropdownMenuItem asChild>
                                    <Link href={RoleController.edit(row.original.id).url} className='flex items-center gap-2 text-sm'>
                                        Edit
                                    </Link>
                                </DropdownMenuItem>
                                <DropdownMenuItem
            ...
            <div className='container mx-auto py-8'>
                <div className='flex flex-col gap-4 md:flex-row md:items-center md:justify-between'>
                    <div>
                        ...
                        <p className='text-muted-foreground'>Manage roles in one place.</p>
                    </div>
                    <div className='flex flex-wrap items-center gap-2 md:flex-nowrap'>
                        <Button asChild>
                            <Link href={RoleController.create().url}>New Role</Link>
                        </Button>
                    </div>
                </div>
                <DataTable<RoleRecord>
                    ...
                    enableColumnFilters
                    enableMultiSort
                    routeFunction={RoleController.index}
                    resetRoute={RoleController.index().url}
                    emptyMessage='No roles found'
                    emptyDescription='Try adjusting your filters or create a new role'
                />
            </div>
        </AppLayout>
    );
